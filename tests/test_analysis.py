...
def test_data(tmp_path):
    ...
    data_path = tmp_path / "test_creditcard.csv" #change to filtered data later
    data_path.write_text(data)
    return str(data_path)

def test_analysis_output_is_dataframe(spark, test_data, tmp_path):
    actual = analysis(spark, test_data)
...
def test_analysis_column_output(spark, test_data, tmp_path):
    actual = analysis(spark, test_data)
    for df in actual.values():
